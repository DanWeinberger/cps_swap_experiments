...
carr_ps2 <- carr_ps1 %>%
  filter(!is.na(GlcA)) %>%
  mutate(logoffset=log(carriage_samples),
         period=as.factor(period),
         period = relevel(period, ref='pre.PCV'))

mod1 <- glm.nb(carriage ~  vt + GlcA + GalA+ GlcNAc + GalNAc+Rha + study + offset(logoffset),  data=carr_ps2)

summary(mod1)

mod2 <- glm.nb(carriage ~  vt + Rha + period + Rha*period + offset(logoffset),  data=carr_ps2)
