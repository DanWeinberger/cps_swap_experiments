...
st.avg.max.od <- comp1 %>%
  ...
  summarize(max_od = mean(max_od, na.rm=T)) %>%
  rename(st=Serotype) %>%
  ungroup() %>%
  reshape2::dcast(.,st~Background, value.var='max_od') %>%
...
```{r}
##Carriage
mod1b <- glm.nb(carriage ~ beta_back22f +study , data=carr2)
summary(mod1b)
...
ggplot(carr2, aes(x=beta_back22f, y=log_case_carr, color=study)) +
  geom_point()+
  theme_classic() +
  facet_wrap(~study)
