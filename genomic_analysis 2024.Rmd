...
gene_key <- read.csv('./Data/pangenome/gene_presence_absence.csv') %>%
  dplyr::select(Gene, Non.unique.Gene.name,Annotation)

g0 <- read.table(file='./Data/pangenome/gene_presence_absence.Rtab', header=T) %>%
...
loc1 <- read.csv('./Data/DBB51US6016-Alignment-HitTable.csv', header=F) %>%
  ...
  filter(pct_ident>90 & pct_coverage >90) %>%
  select(allele1, start, end,pct_ident,pct_coverage,query_length) %>%
  mutate(allele1 = gsub('gene_','',allele1),
           allele=if_else( grepl('group', allele1),str_extract(allele1, "^[^_]+_[^_]+") ,  str_extract(allele1, "^(.*?)(?=_)"))
  )
# 
# p1a <- p1$gain_loss %>%
...
p1a <- p2$gain_loss %>%
  ...
  mutate(allele=if_else( grepl('group', allele1),str_extract(allele1, "^[^_]+_[^_]+") ,  str_extract(allele1, "^(.*?)(?=_)"))) %>%
  right_join(loc2, by='allele') %>%
  filter(value %in% c(-1,1) & !is.na(start)) %>%
  mutate(value=as.factor(value) ,strainN= as.numeric(as.factor(strain))+rnorm(n(), 0, 0.05) )
...
p2b <- ggplot(p1b, aes( y=strainN, x=start ,color=value,text=paste(strain,allele), alpha=0.5 )) +
         geom_point(size=0.3) +
       theme_classic() +
    xlim(range(loc1$start) ) +
  geom_vline(xintercept=c(315597, 339358)) #roughly dexB and aliA
...
```{r}
out.list <- list('presence_15bc'=p2, 'presence_22f'=p3,'map15bc'=p2a,
         'map22f'=p2b)
saveRDS(out.list,'./data/plots.rds')
```
