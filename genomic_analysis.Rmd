...
g1.labs <- read.table(file = './Data/Group1 gene.presence_absence.tsv', sep = '\t', header = TRUE) %>%
  row.names()

g2.labs <- read.table(file = './Data/Group2 gene.presence_absence.tsv', sep = '\t', header = TRUE)%>%
  row.names()

g3.labs <- read.table(file = './Data/Group3 gene.presence_absence.tsv', sep = '\t', header = TRUE)%>%
  row.names()
...
g2 <- g0 %>%
  dplyr::select( gene_name_cluster, all_of(intersect(g2.labs, names(g0)))) %>%
  reshape2::melt(., id.vars=c('gene_name_cluster')) %>%
  reshape2::dcast(variable ~ gene_name_cluster) 
...
g2 <- g2[,prev_acq_g2>0 & prev_acq_g2<1]

##same for g3

g3 <- g0 %>%
  dplyr::select( gene_name_cluster, all_of(intersect(g3.labs, names(g0)))) %>%
  reshape2::melt(., id.vars=c('gene_name_cluster')) %>%
  reshape2::dcast(variable ~ gene_name_cluster) 
...
a1 <- read.table(file = './Data/Genes.with.annotations.tsv', sep = '\t', header = TRUE) %>%
  ...
  ungroup() %>%
  select(allele,Cps.Gene,gene_name_cluster, Gene,Non.unique.Gene.name,Cps.gene.name ,Sequence)
